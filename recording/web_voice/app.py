...
from voice_assist import write_wav
...
def getConvResponse():
    ...
    response_text = ["re", "ro", "re", "ro", "re", "ro", "..."]
    response_context = "Fool"
    
    responseDetails = {'responseText': ' '.join(response_text),
...
def getSpeechFromText():
    ...
    # TODO: do nothing or play pre-recorded audios (no TTS plan)
...
def getTextFromSpeech():
    # TODO: replace with Azure or CMUH-ASR
    ...
    print(audio)
    print(len(audio))
    wav_file = os.path.join("tmp", str(uuid1())+".wav")
    write_wav(audio, wav_file)
    text_output = "ASR: to be implemented"
